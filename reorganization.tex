...
If special odd orthogonal group case (that is, $\epsilon_{\hermitianSpace} = 1$, $\quadraticExtension = \finiteField$ and $\dim_{\finiteField} \hermitianSpace$ is odd) and in the unitary case (that is, $\quadraticExtension \ne \finiteField$), we have that $\algebraicGroup{G}$ has connected center and we denote $\tilde{\algebraicGroup{G}} = \algebraicGroup{G}$.
...
A \emph{rational torus} $\algebraicGroup{T} \subseteq \algebraicGroup{G}$ over $\mathbb{F}$ is a torus defined over $\mathbb{F}$. 
Over a finite field, $\algebraicGroup{G}(\mathbb{F})$ conjugacy classes of maximal rank rational tori \calvin{check if we need connected or something} are parameterized by  Frobenius order twisted conjugacy classes in the Weyl group. In particular if you fix a rational torus $\algebraicGroup{T}_0$ with rank $=\mathrm{rk}(\algebraicGroup{G}$ and an element $g \in \algebraicGroup{G}(\algebraicClosure{\mathbb{F}})$ such that $g^{-1}\Frobenius(g) \in N(\algebraicGroup{T}_0)$ then the torus $g\algebraicGroup{T}_0g^{-1}$ is rational, and it's $\algebraicGroup{G}(\mathbb{F})$-conjugacy class is determined  by the Frobeinus-twisted conjugacy class of the image of $g^{-1}\Frobenius(g)$ in $W(\algebraicGroup{T}_0)(\algebraicClosure{\mathbb{F}})$ [cite Roger Carter Finite Groups of Lie Type section 3.3].%since $\Frobenius(g\algebraicGroup{T}_0g^{-1}) = g\algebraicGroup{T}_0g^{-1}$
...
Applying this theory to our groups $\algebraicGroup{G}$, for each group we find a nice rational torus $\algebraicGroup{T}$ and write out the Frobenius order action on the Weyl group and the 

\begin{enumerate}
	\item(Special orthogonal groups) 
	In each case let $\algebraicGroup{T}$ be the diagonal torus. Then $W(\algebraicGroup{T} )$ has representatives the type B or D permutation matrices which are fixed by the Frobenius order.
	\begin{enumerate}[(a)]
		\item (Split odd case $\algebraicGroup{SO}_{2n+1}$) $\algebraicGroup{T} = \algebraicGroup{G}_m^{n}$. In type B the Weyl group conjugacy classes are given by $(\lambda^+,\lambda^-)$ where $\lambda^+$ is the partition formed from and $\lambda^-$ is the partition formed from. Given a representation $w$ of the conjugacy class we get:
		$$^w\algebraicGroup{T} = \Pi_{\lambda^+_i} \mathrm{Res}_{\mathbb{F}_{\lambda^+_i}:\mathbb{F}}\algebraicGroup{G}_m \times \Pi_{\lambda^-_i} \mathrm{Res}_{\mathbb{F}_{\lambda^-_i}:\mathbb{F}}\algebraicGroup{U}(1)$$
		\item (Split even case $\algebraicGroup{SO}_{2n}^+$) $\algebraicGroup{T} = \algebraicGroup{G}_m^{n}$.  Given $w \in W(\algebraicGroup{T})$ the rational structure of $^w\algebraicGroup{T}$ is determined by the conjugacy class of $w$ in the Type B Weyl group (a $\mathbb{Z}/2\mathbb{Z}$ extension of $W(\algebraicGroup{T})$. If $w$ is conjugate in the extension to $(\lambda^+,\lambda^-)$ where $\lambda^+$ is the partition formed from and $\lambda^-$ is the partition formed from:
		$$^w\algebraicGroup{T} = \Pi_{\lambda^+_i} \mathrm{Res}_{\mathbb{F}_{\lambda^+_i}:\mathbb{F}}\algebraicGroup{G}_m \times \Pi_{\lambda^-_i} \mathrm{Res}_{\mathbb{F}_{\lambda^-_i}:\mathbb{F}}\algebraicGroup{U}(1)$$
			\item (Non-split even case $\algebraicGroup{SO}_{2n}^-$) Set $\algebraicGroup{T} = \algebraicGroup{G}_m^{n-1} \times \algebraicGroup{U}(1)$ the diagonal torus. Given $w \in W(\algebraicGroup{T})$ the rational structure of $^w\algebraicGroup{T}$ is determined by the conjugacy class of $w$ in the Type B Weyl group (a $\mathbb{Z}/2\mathbb{Z}$ extension of $W(\algebraicGroup{T})$. If $w \circ (-1,1)$ is conjugate in the extension to $(\lambda^+,\lambda^-)$ where $\lambda^+$ is the partition formed from and $\lambda^-$ is the partition formed from:
		$$^w\algebraicGroup{T} = \Pi_{\lambda^+_i} \mathrm{Res}_{\mathbb{F}_{\lambda^+_i}:\mathbb{F}}\algebraicGroup{G}_m \times \Pi_{\lambda^-_i} \mathrm{Res}_{\mathbb{F}_{\lambda^-_i}:\mathbb{F}}\algebraicGroup{U}(1)$$
	\end{enumerate}
	\item (Symplectic groups) In type B the Weyl group conjugacy classes are given by $(\lambda^+,\lambda^-)$ where $\lambda^+$ is the partition formed from and $\lambda^-$ is the partition formed from. Given a representation $w$ of the conjugacy class we get:
		$$^w\algebraicGroup{T} = \Pi_{\lambda^+_i} \mathrm{Res}_{\mathbb{F}_{\lambda^+_i}:\mathbb{F}}\algebraicGroup{G}_m \times \Pi_{\lambda^-_i} \mathrm{Res}_{\mathbb{F}_{\lambda^-_i}:\mathbb{F}}\algebraicGroup{U}(1)$$
	\item (Unitary groups) We let our torus be the diagonal torus for the unitary group with hermitian form $I_n$. Then $\algebraicGroup{T}=U(1)^n$ and the Frobenius action on the Weyl group is trivial. Note since all hermitian forms are equivalent over a finite field, $\algebraicGroup{T}$ is a torus in the group $\algebraicGroup{U}_n$ from the previous section.  In type A the Weyl group conjugacy classes are given by $(\lambda)$, and
	$$^w\algebraicGroup{T} = \Pi_{\lambda_i} \begin{cases}
	\mathrm{Res}_{\mathbb{F}_{\lambda^+_i}:\mathbb{F}} \algebraicGroup{G}_m& \lambda_i \textnormal{ is even} \\
	\mathrm{Res}_{\mathbb{F}_{\lambda^+_i}:\mathbb{F}}\algebraicGroup{U}(1) & \lambda_i \textnormal{ is odd}
	\end{cases}$$
	\item (Even special orthogonal similitude groups)
	\item (Symplectic similitude groups)
\end{enumerate}
...
\begin{tabular}{|c|c|}
	\hline 
		$\algebraicGroup{G}$ & $\algebraicGroup{T}^{\mathrm{std}}_{\algebraicGroup{G}}$ \tabularnewline
		\hline 
		\hline 
...
\hline 
	...
	\end{tabular}
\end{center}
Here $\algebraicGroup{T}^{\mathrm{std}}_{\algebraicGroup{G}}$ is split for all rows except for the the last row.
Here $\algebraicGroup{T}^{\mathrm{std}}_{\algebraicGroup{G}}$, is split for all rows except for the the last row. We also write down the standard tori for the relevant corresponding similitude groups.
\begin{center}
	\begin{tabular}{|c|c|}
		\hline 
		$\algebraicGroup{G}$ & $\algebraicGroup{T}^{\mathrm{std}}_{\algebraicGroup{G}}$ \tabularnewline
		\hline 
		\hline 
		$\algebraicGroup{\GSp}_{2n}$ or $\algebraicGroup{\GSO}^{+}_{2n}$ & $\left\{\left(\lambda, \begin{pmatrix}
			\lambda a\\
			& a^{\ast}
		\end{pmatrix}\right) \mid a \in \algebraicGroup{A}_n,\,\, \lambda \in \multiplcativeScheme \right\}$ \tabularnewline
		\hline 
		$\algebraicGroup{\GSO}_{2n}^{-}$ & $\left\{ \left(\algebraicGroup{\aFieldNorm}_{\finiteFieldExtension{2} \slash \finiteField}\left(h\right), \begin{pmatrix}
			\algebraicGroup{\aFieldNorm}_{\finiteFieldExtension{2} \slash \finiteField}\left(h\right) a\\
			& h\\
			& & a^{\ast}
		\end{pmatrix}\right) \mid a \in \algebraicGroup{A}_{n-1}\,\, h \in \restrictionOfScalars{\finiteFieldExtension{2}}{\finiteField}{\multiplcativeScheme} \right\}$ \tabularnewline
		\hline		
	\end{tabular}
\end{center}
Here we realize $\restrictionOfScalars{\finiteFieldExtension{2}}{\finiteField}{\multiplcativeScheme}$ as $$\restrictionOfScalars{\finiteFieldExtension{2}}{\finiteField}{\multiplcativeScheme} = \left\{ \left(\lambda, \begin{pmatrix}
	a & b\\
	db & a
\end{pmatrix}\right) \mid a^2 - db^2 = \lambda \in \multiplcativeScheme \right\},$$
where $d$ is as in \Cref{subsec:isometry-groups}.

\elad{Under construction}
If $m \ge 1$, we denote 
$$\NormOneGroup{2m} := \left\{ x \in \multiplicativegroup{\finiteFieldExtension{2m}} \mid \FieldNorm{2m}{m}\left(x\right)= 1\right\} = \left(\mathrm{Res}_{\mathbb{F}_{m}:\mathbb{F}} \algebraicGroup{U}(1)\right)(\mathbb{F}).$$
...
\begin{example}
	\elad{Correct example or remove it}
