...
\begin{definition}
    ...
    The pairs $(T',\theta'),(T,\theta)$ are said to be \emph{geometrically conjugate} if the geometric conjugacy classes $(s_{T,\theta})$ and $(s_{T,\theta'})$ are equal.
    
    The \emph{Lusztig series} $\LusztigSeries{G}{s}$ associated to a Frobenius stable geometric conjugacy class $(s) \subset \algebraicGroup{G}^*$ consists of the (equivalence classes of the) irreducible representations $\pi$ which appear with non-zero multiplicity in $R_{T, \theta}$ for some $(T,\theta)$ such that $(s_{T,\theta}) = (s)$.
\end{definition}

\begin{center}
\begin{tabular}{ c c c c c c}
 Group & Fixed Torus & Frobenius on $X(T)$ & Frobenius on $W$ &Dual Group & Dual Torus \\ 
 $\algebraicGroup{\GSp}_{2n}$ & $(\multiplicativegroup{\finiteField})^{n+1}$ & 1 & 1 &$\algebraicGroup{\GSpin}_{2n+1}$ &  $(\multiplicativegroup{\finiteField})^{n+1}$ \\
 $\algebraicGroup{\GSO}^{\pm}_{2n}$ & $(\multiplicativegroup{\finiteField})^{n+1}$ & 1 & 1 & $\algebraicGroup{\GSpin}^{\pm}_{2n}$ &  $(\multiplicativegroup{\finiteField})^{n+1}$ \\ $\algebraicGroup{\SO}_{2n+1}$ & $(\multiplicativegroup{\finiteField})^{n+1}$ &1 & 1 &$\algebraicGroup{\Sp}_{2n}$ &  $(\multiplicativegroup{\finiteField})^{n+1}$ \\ 
  $\algebraicGroup{\UnitaryGroup}_n$ & $(\NormOneGroup{2})^{n}$ & -1 & 1 & $\algebraicGroup{\UnitaryGroup}_n$ &  $(\NormOneGroup{2})^{n}$ 
\end{tabular}
\end{center}
...
\begin{lemma}\label{lem:indep_geo_conj}If $\left(T, \theta\right)$ and $\left(T',\theta'\right)$ are geometrically conjugate, then
    $$\frac{\dblVirtualJacobiSumScalar{R_{T, \theta}}{\chi}}{R_{T,\theta}(1)} = \frac{\dblVirtualJacobiSumScalar{R_{T, \theta'}}{\chi}}{R_{T',\theta'}(1)}.$$
\end{lemma}
...
\begin{enumerate}[(i)]
	...
	\]
	which by the Hasse-Davenport relation depends only on the geometric conjugacy class of $(T,\theta)$ hence the semisimple conjugacy class of $(s_{T,\theta})$.
...
\begin{tabular}{ c c}
 ...
 $\GSp_{2n}$  & $\GSpin_{2n+1} \to \GL_{2n+1}$ \\
 $\GSO^{\pm}_{2n}$ & $\GSpin_{2n}^{\pm} \to \GL_{2n}$ \\
 $\SO_{2n+1}$ &$\Sp_{2n} \to \GL_{2n}$ \\
 $\UnitaryGroup_n$  & $\UnitaryGroup_n \to \GL_{n}$  
\end{tabular}
\end{center}
...
\label{lem:cochar_computation}
    Define $\phi:(\mathbb{Q}/\mathbb{Z})_{p'}^n \to (\mathbb{Q}/\mathbb{Z})_{p'}^{2n} = X_*(\tilde{\algebraicGroup{T}}_{\GL_{2n}})_{(\mathbb{Q}/\mathbb{Z})_{p'}}$ by $\phi(x) = (x,-x)$. 
    \begin{enumerate}
        \item Suppose that $\Frobenius$ acts by trivially on both the domain and the range. Then $$x_{\finiteFieldExtension{n}^\times \times \finiteFieldExtension{n}^\times,\alpha \times \alpha^{-1}} = \phi(x_{\finiteFieldExtension{n},\alpha}).$$
        \item Suppose that $\Frobenius$ acts by $-1$ on the domain and by $\begin{pmatrix}
            0 & 1 \\
            1 & 0\end{pmatrix}$ on the codomain. Then $$x_{\finiteField_{2n}^\times,\theta'}=\phi(x_{\NormOneGroup{2n},\theta})$$ for $\theta'(t) = \theta(t^{1-q^n})$.
    \end{enumerate}
\end{lemma}
\begin{proof}
Recall that $x_{T,\theta}$ is defined by for sufficiently large $m$ with $\zeta_m = 1/(q^m-1)$ under the prescribed identification $\multiplicativegroup{\algebraicClosure{\finiteField}} \cong (\mathbb{Q}/\mathbb{Z})_{p'}$ by the formula.\[
    \exp(\langle x_{T,\theta},y\rangle) = \theta(y(\aFieldNorm_{\algebraicGroup{T}(\finiteField_m):\algebraicGroup{T}(\finiteField)}(y(\zeta_m))),
\]
for all $y \in X_{\ast}\left(\algebraicGroup{T}\right)$.

Thus the result follows from the following computations.
\begin{enumerate}
	\item Let $\left(y_1, y_2\right) \in X_{\ast}\left(\GL_n\right) \times X_{\ast}\left(\GL_n\right)$. Then  \begin{align*}
