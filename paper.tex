...
\begin{enumerate}
	\item (Even special orthogonal groups): $\quadraticExtension = \finiteField$ and $\epsilon_{\hermitianSpace} = 1$. Denote $\dim_{\finiteField} \hermitianSpace = 2n$. In this case, we have that $G$ consists of all elements $g \in \IsometryGroup\left(\hermitianSpace\right)$ with $\det \hermitianSpace = 1$, and that $\GroupExtension{G}$ consists of all elements $g \in \GroupExtension{\IsometryGroup\left(\hermitianSpace\right)}$ such that $\det g = \similitudeCharacter\left(g\right)^n$ \calvin{not sure I get what this $\Tilde{G}$ is. I thought it would just be $\widetilde{Isom(V)}$-or maybe its identity component. As it reads it sounds like you are supposed to fix sim and then define $\tilde{G}$.} \elad{This is the definition used is Lusztig's paper for example. It is the identity component of $\GroupExtension{\IsometryGroup\left(\hermitianSpace\right)}$. Generally speaking, we have the equality $\det g^2 = \similitudeCharacter\left(g\right)^{2n}$ (you can see this by taking the determinant of both sides). So the identity component is the matrices for which the ``square root'' of this equality holds, i.e. $\det g = \similitudeCharacter\left(g\right)^n$ and not $\det g = -\similitudeCharacter\left(g\right)^n$} There are two cases here.
	\begin{enumerate}
		...
		The group $\GroupExtension{G}$ is isomorphic to the subgroup $\GSO_{2n}\left(\finiteField\right)$ of $\GL_{2n}\left(\finiteField\right)$, consisting of elements $g$ such that $$g \cdot w_{2n} \cdot \transpose{g} = \similitudeCharacter\left(g\right) w_{2n},$$
		and such that $\det g = \similitudeCharacter\left(g\right)^n$, where $\similitudeCharacter\left(g\right)$ is a non-zero scalar.
		\item Non-split case: \calvin{do we want to do even characteristic? Not sure what the classification of quadratic forms over even char are} \elad{Let's not do characteristic two} for odd characteristic there is one non-split case when $V = X \oplus \mathbb{E} \oplus Y$ where $\mathbb{E}$ is the unique degree two extension of $\mathbb{F}$ with quadratic form $Nm_{\mathbb{E}:\mathbb{F}}$ \calvin{find someone to cite}
	\end{enumerate}
...
where $\hermitianSpace' \subset \hermitianSpace$ is a non-degenerate subspace, orthogonal to $\xIsotropic$ and $\yIsotropic$. Let $P = P_{H}\left(\mathcal{F}\right)$ be the parabolic subgroup of $H$, consisting of all elements stabilizing the flag \begin{equation}\label{eq:definition-of-flag-for-parabolic-subgroup}
	\mathcal{F} \colon 0 \subset \xIsotropic \subset \xIsotropic \oplus \hermitianSpace' \subset \xIsotropic \oplus \hermitianSpace' \oplus \yIsotropic = \hermitianSpace.
\end{equation}
...
\begin{proof}
	...
	Hence, \begin{equation}
		\begin{split}\label{eq:gamma-factor-multiplicativity-support-expression}
			& \chi\left(-1\right)^k q^{k^2} \sum_{p \in P} \Phi_{\chi}\left(x^{-1} p\right) \left(\tau \overline{\otimes} \pi'\right)\left(p\right) \\
			= & \sum_{A \in \EndomorphismRing_{\quadraticExtension}\left(\hermitianSpace\right)} \sum_{h \in \GL_{\quadraticExtension}\left(\hermitianSpace\right)} \chi\left(\detQuadratic h\right)\fieldCharacter_{\quadraticExtension}\left(\trace\left(Ah\right)\right) \sum_{p \in P} \fieldCharacter_{\quadraticExtension}\left(\trace\left(A \left(x^{-1}p - \idmap_{\hermitianSpace}\right)\right)\right) \left(\tau \overline{\otimes} \pi'\right)\left(p\right).
		\end{split}
	\end{equation}
	...
	\end{align*}
	Replace the sum over $p \in P$ by a sum over $l \in L$ and $u \in N$ where $p = lu$ and change variables $u \mapsto u^{-1}$ and $A \mapsto A u$. We arrive at the sum
	\begin{align*}
		\sum_{h \in \GL_{\quadraticExtension}\left(\hermitianSpace\right)} \chi\left(\detQuadratic h\right)\fieldCharacter_{\quadraticExtension}\left(\trace h\right) \sum_{A \in \GL_{\quadraticExtension}\left(\hermitianSpace\right)} \chi^{-1}\left(\detQuadratic A\right) \sum_{l \in L} \sum_{u \in U} \fieldCharacter_{\quadraticExtension}\left(\trace\left(A \left(x^{-1} l - u\right)\right)\right) \left(\tau \overline{\otimes} \pi'\right)\left(l\right).
	\end{align*}
	Using the appendix, we arrive at the sum
	\eqref{eq:gamma-factor-multiplicativity-support-expression} is given by
	\begin{align*}
		& \sum_{h \in \GL_{\quadraticExtension}\left(\hermitianSpace\right)}  \chi\left(\detQuadratic\left(xh\right)\right) \fieldCharacter_{\quadraticExtension}\left(\trace h\right) \\
		& \times \sum_{A \in P_{\GL_{\quadraticExtension}\left(\mathcal{F}\right)}} \sum_{p \in P} \chi^{-1}\left(\detQuadratic A\right) \fieldCharacter_{\quadraticExtension}\left(\trace\left(A \left(x^{-1} p - \idmap_{\hermitianSpace} \right)\right)\right) \left(\tau \overline{\otimes} \pi'\right)\left(p\right),
	\end{align*}
	Where $P_{\GL_{\quadraticExtension}\left(\hermitianSpace\right)}\left(\mathcal{F}\right)$ is the parabolic subgroup of $\GL_{\quadraticExtension}\left(\hermitianSpace\right)$ stabilizing the flag $\mathcal{F}$ from \eqref{eq:definition-of-flag-for-parabolic-subgroup}. By decomposing the sum over $A$ into a sum over its Levi part and its unipotent radical, we see that the inner sum over the unipotent radical will vanish unless $x^{-1} p - \idmap_{\hermitianSpace}$ is an upper triangular matrix, which implies that $x \in P \cap G$.
...
\end{proof}
\begin{remark}
	A proof similar to the second part of our proof is given in \cite{Chang1997}. However, the proof in \cite{Chang1997} only works if the parabolically induced representation is irreducible.
\end{remark}
...
\calvin{non split form of $SO(even)$, is the dual group itself?. The torus I think is $(\mathbb{F}_2)^{Nm=1} \times (\mathbb{F}^\times)^{dim -1}$. The Frobenius action on the characters is $(-1,1,1,1\hdots 1)$ and on $W$ its trivial}
...
\begin{definition}
	A \emph{stable function} on $\algebraicGroup{G}(\finiteField)$ is a class function $f$ such that $\sum_{g \in G} f(g)(g)$ is a scalar which only depends on the Lusztig packet for $\pi$. \calvin{FUNCTORIAL TRANSFER FOR REDUCTIVE GROUPS AND CENTRAL
...
\label{thm:Phi_stable}
	$\Phi_{\chi}(g)$ is a stable function. In other words $\dblVirtualJacobiSumScalar{\pi}{\chi}$ only depends on the Lusztig packet of $\pi$.
...
\label{lem:indep_geo_conj}
    $\frac{\dblVirtualJacobiSumScalar{R_{T, \theta}}{\chi}}{R_{T,\theta}(1)}$ only depends on the geometric conjugacy class of $(T,\theta)$.
\end{lemma}
...
\subsection{Computation for Lusztig Series}
The goal of this section is to prove \Cref{thm:Phi_stable} in particular that if $c_{\pi}$ is nonzero in the virtual representation $R_T(\theta) = \sum c_{\pi} \pi$ then
\begin{theorem}
...
\subsubsection{Proof of \Cref{thm:Phi_stable}}
...
\appendix
\section{Unipotent averaging}

Choose bases $e_1,\dots,e_k$ for $\xIsotropic$ and $f_1,\dots,f_k$ for $\yIsotropic$ that are dual, i.e., $\innerproduct{e_i}{f_j} = \delta_{ij}$, where $\delta_{ij}$ is Kronecker's delta function, and choose a basis $v'_1, \dots, v'_{r}$ for $\hermitianSpace'$. Then with respect to the basis $e_1,\dots,e_k,v_1',\dots,v_r',f_k,f_{k-1},\dots,f_1$, elements in $N$ can be represented in matrix form $$\begin{pmatrix}
	\IdentityMatrix{k} & X & Z\\
	& \IdentityMatrix{r} & Y\\
	& & \IdentityMatrix{k}
\end{pmatrix},$$
where $X,Y$ and $Z$ are matrices satisfying certain relations. Under this realization, the center $\Center\left(N\right)$ of $N$ corresponds to matrices of the form \begin{equation}\label{eq:center-matrix-representation}
	\begin{pmatrix}
		\IdentityMatrix{k} & 0 & Z\\
		& \IdentityMatrix{r} & 0\\
		& & \IdentityMatrix{k}
	\end{pmatrix},
\end{equation}
where $Z$ is a certain type of matrix: it is ``-$\epsilon$ symmetric'', that is, it satisfies $$Z = -\epsilon w_k \involution{\left(\transpose{Z}\right)} w_k,$$
where $$w_k = \begin{pmatrix}
	& & & 1\\
	& & 1\\
	& \iddots\\
	1
\end{pmatrix} \in \GL_k\left(\finiteField\right)$$ is the long Weyl element.

The following sum appears as an inner sum in \eqref{TODO}:
\begin{equation}\label{eq:sum-that-vanishes-unless-A-is-upper-triangular}
	\sum_{u \in N} \fieldCharacter_{\quadraticExtension}\left(\trace\left(-Au\right)\right) = \sum_{u \in N \slash \Center\left(N\right)} \fieldCharacter_{\quadraticExtension}\left(\trace\left(-Au\right)\right) \sum_{z \in \Center\left(N\right)} \fieldCharacter_{\quadraticExtension}\left(\trace\left(-Az\right)\right).
\end{equation}
Consider the inner sum 
\begin{equation*}
	\sum_{u \in Z\left(N\right)} \fieldCharacter_{\quadraticExtension}\left(\trace\left(-Az\right)\right).
\end{equation*}
This sum vanishes unless the matrix representation of $A$ is of the form $$\begin{pmatrix}
	a_{k \times k} & a'_{k \times r} & a''_{k \times k} \\
	b_{r \times k} & b'_{r \times r} & b''_{r \times k}\\
	c_{k \times k} & c'_{k \times r} & c''_{k \times k}
\end{pmatrix},$$
where $c_{k \times k}$ is ``$\epsilon$-symmetric'', that is, it satisfies \begin{equation}\label{eq:epsilon-symmetric-relation}
	c_{k \times k} = \epsilon w_k \involution{\left(\transpose{c_{k \times k}}\right)} w_k.
\end{equation}
Let us denote by $R$ the subset of $\EndomorphismRing_{\quadraticExtension}\left(\hermitianSpace\right)$ consisting of elements such that their matrix representation satisfies the relation \eqref{eq:epsilon-symmetric-relation}.
Then we may replace the sum
$$\sum_{A \in \GL_{\quadraticExtension}\left(\hermitianSpace\right)} \chi^{-1}\left(\detQuadratic A\right) \sum_{u \in N} \fieldCharacter_{\quadraticExtension}\left(\trace\left(A \left(x^{-1} l - u\right)\right)\right)$$ with 
$$\sizeof{\Center\left(N\right)} \sum_{A \in \GL_{\quadraticExtension}\left(\hermitianSpace\right) \cap R} \chi^{-1}\left(\detQuadratic A\right) \sum_{u \in N \slash \Center\left(N\right)} \fieldCharacter_{\quadraticExtension}\left(\trace\left(A \left(x^{-1} l - u\right)\right)\right).$$
Let $N_{\GL_{\quadraticExtension}\left(\hermitianSpace\right)}\left(\mathcal{F}\right)$ be the unipotent radical of the parabolic subgroup $P_{\GL_{\quadraticExtension}\left(\hermitianSpace\right)}\left(\mathcal{F}\right)$ of $\GL_{\quadraticExtension}\left(\hermitianSpace\right)$ stabilizing the flag $\mathcal{F}$. Its center $\Center\left(N_{\GL_{\quadraticExtension}\left(\hermitianSpace\right)}\left(\mathcal{F}\right)\right)$ consists of linear maps whose matrix representation is of the form \eqref{eq:center-matrix-representation}, where $Z$ is an arbitrary matrix.

We notice the following observation: if $A \in \GL_{\quadraticExtension}\left(\hermitianSpace\right) \cap R$, then for any $z \in \Center\left(N_{\GL_{\quadraticExtension}\left(\hermitianSpace\right)}\left(\mathcal{F}\right)\right)$, we have $zA \in \GL_{\quadraticExtension}\left(\hermitianSpace\right) \cap R$. This is because the left action of $\Center\left(N_{\GL_{\quadraticExtension}\left(\hermitianSpace\right)}\left(\mathcal{F}\right)\right)$ does not change the last $k$ rows of $A$. It follows that $\GL_{\quadraticExtension}\left(\hermitianSpace\right) \cap R$ is a union of left (or right) cosets of $\Center\left(N_{\GL_{\quadraticExtension}\left(\hermitianSpace\right)}\left(\mathcal{F}\right)\right)$.




$P_{\GL_{\quadraticExtension}\left(\hermitianSpace\right)}\left(\mathcal{F}\right)$ is the parabolic subgroup of $\GL_{\quadraticExtension}\left(\hermitianSpace\right)$ that stabilizes the flag $\mathcal{F}$ from \eqref{eq:definition-of-flag-for-parabolic-subgroup}.
