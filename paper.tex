...
\label{lem:cochar_computation}
    Define $\phi:(\mathbb{Q}/\mathbb{Z})_{p'}^n \to (\mathbb{Q}/\mathbb{Z})_{p'}^{2n} = X_*(GL_{2n})_{(\mathbb{Q}/\mathbb{Z})_{p'}}$ by $\phi(x) = (x,-x)$.
    \begin{enumerate}
        \item $x_{\mathbb{F}_n^\times \times \mathbb{F}_n^\times,\alpha \times \alpha^{-1}} = \phi(x_{\mathbb{F}_n,\alpha})$
        \item $x_{\mathbb{F}_{2n}^\times,\theta'}=\phi(x_{(\mathbb{F}_{2n})^{Nm=1},\theta})$
    \end{enumerate}
    where $\theta'(t) = \theta(t^{1-q^n})$.
\end{lemma}
\begin{proof}
...
\[
    exp(\langle x_{T,\theta},y\rangle) = \theta(y(Nm_{T(\mathbb{F}_m):T(\mathbb{F})}(y(\zeta_m))).
\]
