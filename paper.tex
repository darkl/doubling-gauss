...
\label{prop:central_char}
    Any function supported on the center of $\algebraicGroup{G}(\finiteField)$ is a stable function. \calvin{maybe add in connected group} 
\end{proposition}
\begin{proof}
...
\begin{proof}
\begin{enumerate}[(i)]
	\item In this case when $T = \prod_{i} \finiteFieldExtension{n_i}$ and $\theta = \prod_{i} \alpha_i$, we recover the Gauss sum
	\[
