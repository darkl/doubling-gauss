...
Let $\mathbb{G}$ be a reductive group over $\mathbb{F}$ and fix a torus $\mathbb{T}$ defined over $\mathbb{F}$. A \emph{dual group over $\mathbb{F}$} $\mathbb{G}^*$ with a dual torus $\mathbb{T}^*$ is a group with an isomorphism $X_*(T) \cong X^*(T^*)$ which is compatible with the action of Frobenius and sends roots to coroots \calvin{check if I flipped upper and lower star}.

Associated with a torus character pair $(T,\theta)$ is a  Frobenius stable $W(\mathbb{T})$ orbit in $X_*(\mathbb{T}) = X^*(\mathbb{T}^*)$. Under a choice of $\overline{\mathbb{F}}^\times \cong \left(\mathbb{Q}/\mathbb{Z}\right)_{p'}$, $X^*(\mathbb{T}^*) \otimes \left(\mathbb{Q}/\mathbb{Z}\right)_{p'} \cong \mathbb{T}^*(\overline{\mathbb{F}})$, this associates to $(T,\theta)$ a Frobenius stable semisimple conjugacy class in $\mathbb{G}^*$.
