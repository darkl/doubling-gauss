...
\end{theorem}
\begin{proof}
	We will prove this by induction on $\dim \hermitianSpace$. Let us keep the notation as in \Cref{thm:computation-of-doubling-gauss-sum-scalar-for-deligne-lusztig-characters}. If $\left(T,\theta\right)$ is in general position, then $\varepsilon_{\algebraicGroup{G}} \varepsilon_{\algebraicGroup{T}} R_{T,\theta} = \trace \pi$ and $$\dim \pi \cdot \dblGaussSumScalar{\pi}{\chi} = \dblVirtualGaussSumScalar{\trace \pi}{\chi} = \varepsilon_{\algebraicGroup{T}} \varepsilon_{\algebraicGroup{G}} \dblVirtualGaussSumScalar{R_{T,\theta}}{\chi}.$$ We also have in this case $$\centralCharacter{\pi}\left(-1\right) = \prod_{j=1}^r \alpha_j\left(-1\right) \cdot \prod_{i=1}^s \theta_j\left(-1\right).$$
	Hence from \Cref{thm:computation-of-doubling-gauss-sum-scalar-for-deligne-lusztig-characters}, $$\dblGaussSumScalar{\pi}{\chi} = \frac{\dblVirtualGaussSumScalar{R_{T,\theta}}{\chi}}{R_{T,\theta}\left(1\right)} = \centralCharacter{\pi}\left(-1\right) g_T\left(\theta, \fieldCharacter\right) \GaussSumScalar{\chi^2}{\fieldCharacter}^{\left\lfloor\frac{\dim \hermitianSpace}{2}\right\rfloor},$$
	which implies that $\dblGammaFactorSpace{\hermitianSpace}{\pi}{\chi}{\fieldCharacter} = g_T\left(\epsilon, \fieldCharacter\right)$.
	
	Suppose that $\left(T,\theta\right)$ is not in general position and that $\pi$ is such that $c_{\pi} \ne 0$. Fix an isomorphism $T^{\ast} \cong T$, where $T$ is the dual torus of $T$ and let $s$ be the image of $\theta$ under this isomorphism. Then $\pi$ belongs to the Lusztig series $\mathcal{E}\left(G, s\right)$. Suppose first that $\pi$ is not cuspidal. Then $\pi$ appears in a Harish--Chandra series of the form $\left(L, \sigma\right)$, where $L$ is a Levi subgroup of $G$ and where $\sigma$ belongs to the Lusztig--Series $\mathcal{E}\left(L, s\right)$. We may write $L \cong \GL_{t_1}\left(\finiteField\right) \times \dots \times \GL_{t_\ell}\left(\finiteField\right) \times G'$, where $G' = \IsometryGroup\left(\hermitianSpace'\right)$. By the multiplicativity property we have
\end{proof}
