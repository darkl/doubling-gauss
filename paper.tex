...
Notice that the assignments $g \mapsto \posJacobiKernel{\chi}\left(g\right)$ and $g \mapsto \negJacobiKernel{\chi}\left(g\right)$ only depends on the semisimple part of $g \in H$. By \cite[Theorem in Section 1.2]{SaitoShinoda2000}, we have the following result.
...
\label{thm:Phi_stable}
	The assignments $H \to \cComplex$ given by $$h \mapsto \begin{dcases}
		\chi\left(\detQuadratic\left(\idmap_{\hermitianSpace} \pm h\right)\right) & h \in G \text{ and } \detQuadratic\left(\idmap_{\hermitianSpace} \pm h\right) \ne 0,\\
		0 & \text{otherwise.}
	\end{dcases}$$ are stable functions. Equivalently, the assignments $$\pi \mapsto \dblJacobiSumScalar{\pi}{\chi}$$ are constant on geometric Lusztig series.
\end{theorem}
