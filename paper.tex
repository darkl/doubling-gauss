...
\begin{definition}
	A \emph{stable function} on $G(\mathbb{F})$ is a class function $f$ such that $\sum_{g \in G} f(g)\pi(g)$ is a scalar which only depends on the Lusztig packet for $\pi$. \calvin{FUNCTORIAL TRANSFER FOR REDUCTIVE GROUPS AND CENTRAL
...
\label{prop:central_char}
    Any function supported on the center of $G(\mathbb{F})$ is a stable function. \calvin{maybe add in connected group} 
\end{proposition}
\begin{proof}
...
\label{thm:indep_geo_conj}
	...
	%Equivalently	$\frac{\dblVirtualGaussSumScalar{R_{T, \theta}}{\chi}}{R_{T,\theta}(1)}$ only depends on the geometric conjugacy class of $(T,\theta)$.
\end{theorem}

In the $GL_n$ case this follows from theorem \ref{thm:gln-doubling-gauss-sum-in-terms-of-kondo} along with \calvin{Hasse-Davenport? Something else?}.
...
\[
	g_{T,\rho}(\theta,\psi,\chi)  = \Pi_i \tau(\alpha_i \times (\chi\circ Nm),\psi).
\]
By the Hasse-Davenport relation depends only on the geometric conjugacy class of $(T,\theta)$ hence the semisimple conjugacy class of $(s_{T,\theta})$ \calvin{maybe write more or less?}. 
\\
...
The idea behind the proof of this theorem \ref{thm:indep_geo_conj} is that the formula from Theorem \ref{thm:computation-of-doubling-gauss-sum-scalar-for-deligne-lusztig-characters} relates $\dblVirtualGaussSumScalar{R_{T, \theta}}{\chi}/R_{T,\theta}(1)$ to $g_{T,\rho}(\theta,\psi,\chi)$ for the appropriate $\rho$ in the table below. 
\begin{center}
\begin{tabular}{ c c}
...
g_{T,\rho}(\theta,\psi,\chi)= g_{T}(\theta,\psi,\chi) \cdot \begin{cases}
    ...
    \end{cases}.
\]
Corollary \ref{cor:geoconj} then implies that $g_T(\theta,\psi,\chi)$ only depends on the geometric conjugacy class of $(T,\theta)$. 
\begin{comment}
