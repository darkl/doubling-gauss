...
\begin{remark}
	...
	\end{align*}
\end{remark}
\subsection{Independence of geometric conjugacy for Gauss sums}
\calvin{here is the section I am adding in.}
...
Fix an isomorphism $\overline{\mathbb{F}}^\times \cong (\mathbb{Q}/\mathbb{Z})_{p'}$ and an embedding $\overline{\mathbb{F}}^\times \cong \overline{\mathbb{Q}_\ell}^\times$. 
We get an isomorphism
\begin{align*}
\label{eq:DL}
0 \to X_*(\mathbb{T})_{(\mathbb{Q}/\mathbb{Z})_{p'}} \to_{F-1} X_*(\mathbb{T})_{(\mathbb{Q}/\mathbb{Z})_{p'}} \to \mathbb{T}(\overline{\mathbb{F}}) \to 0
\end{align*}
with the last map being $y \mapsto N_{\mathbb{F}^n/\mathbb{F}}(y(\zeta))$
for the $\zeta=q^n-1$-root of unity corresponding to $1/(q^n-1) \in (\mathbb{Q}/\mathbb{Z})_{p'}$ and sufficiently large $n$ (large enough so that $\mathbb{T}$ is split).

Hence for a torus character pair $(T',\theta')$ for $G$, we may conjugate $\mathbb{T}'$ to a fixed torus $\mathbb{T}$ and associate to $\theta$ to an element of $X^*(\mathbb{T})_{(\mathbb{Q}/\mathbb{Z})_{p'}}$ by treating $\theta$ as a linear functional on $Y(\mathbb{T})_{(\mathbb{Q}/\mathbb{Z})_{p'}}$ by composing with the map to $\mathbb{T}(\mathbb{F})$. This is well defined up to the Weyl group, and in fact there is a correspondence between $(T',\theta')$ pairs and $F$-stable Weyl group orbits in $X^*(\mathbb{T})_{(\mathbb{Q}/\mathbb{Z})_{p'}}$ [Coloarry 13.9].  By composing with \ref{eq:DL}, we assocaited to a torus character pair a Frobenius stable semisimple conjugacy class in $G^*$.

\begin{definition}
    The pairs $(T',\theta'),(T,\theta)$ are said to be \emph{geometrically conjugate} if their associated semisimple conjugacy classes in $G^*$ are the same.
    The \emph{Lusztig series} $\LusztigSeries{G}{s}$ associated to a Frobenius stable geometric conjugacy class $(s) \subset \mathbb{G}^*$ consists of (equivalence classes of) the irreducible representations $\pi$ which appear with nonzero multiplicity in $R_{T, \theta}$ for some $(T,\theta)$ pair associated to $(s)$.
\end{definition}
...
The pairs $(T,\theta),(T',\theta')$ are said to be geometrically conjugate if they are associated to the same Frobenius stable geometric conjugacy class. The main point of this section is the following:

For a torus $T$ is a classical group $G$ embedded in $GL_n$ via $\rho$, define
\[
    g_{T,\rho}(\theta,\psi,\chi) := \frac{\sum_{t\in T} \theta(t)\chi(det(\rho(t)-1))}{}
\]

\begin{proposition}
\label{prop:geoconj}
Suppose that $(T,\theta)(T',\theta')$ are maximal torus, character pairs of $G$ for a classical group $G$ which are geometrically conjugate. Then $g_{T,\rho}(\theta,\psi,\chi) = g_{T',\rho}(\theta',\psi,\chi)$ where $\rho$ is the natural embedding of $GL_n$
\end{proposition}

\subsubsection{Gauss sums attached to semisimple elements}


\subsubsection{General case of \ref{prop:geoconj}}
First we fix representations $\Phi$ of the dual groups:
\begin{center}
\begin{tabular}{ c c}
 Group & Dual group representations $\Phi$ \\ 
...
This completes the proof of proposition \ref{prop:geoconj}.
